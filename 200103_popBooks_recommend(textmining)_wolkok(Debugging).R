...
# 1. (ISBN아동_2019년_10-12월_월곡꿈그림도서관) 인기 아동도서 상위 10개 수집
## '도서관/지역별 인기대출 도서 조회' API → (도서명, ISBN, 주제분류, 출판사) 추출
lib_code = 111464  # 월곡꿈그림도서관 코드
start_d = '2019-10-01'
end_d = '2019-12-31'
...
any(is.na(c(pop_books, pop_ISBN, pop_classNum, pop_publisher)))  # 정상이면 False


# 2. (2019년_11월_월곡꿈그림도서관) 장서/대출 csv 데이터 로드 & 전처리
## columns : 도서명, 출판사, ISBN, 부가기호, 주제분류, 도서권수, 대출건수
## (도서권수!=0 & 부가기호==7 & 누적대출건수==0) 충족하는 도서 추출 (비인기 아동도서들)
lib_df = read.csv('data/2019_Dec_SeongbookLibs/월곡꿈그림도서관 장서 대출목록 (2019년 12월).csv', stringsAsFactors=F )[c(2, 4, 6, 8, 10, 11, 12)]
...
str(lib_df)  # 도서명, 출판사, ISBN, 주제분류번호
head(lib_df,2)
table(is.na(lib_df))  # 결측치 존재 확인
...
for (idx in 1:length(pop_books)) {
  ...
  ## (1+m < 20)인 경우, 전체 비인기도서들 중 인기도서들의 출판사에서 발행한 책 20권 이하로 추가
  ## 그래도 (1+m < 20)인 경우, 전체 비인기도서들 중 무작위로 추가
  ...
  ## 수집된 목록이 20권 미만인 경우, 인기도서들의 출판사에서 발행한 책 20권 이하로 추가
  if (length(compare_books) < 20) {
    
    tmp_sample_books = lib_df$도서명[ lib_df$출판사 %in% unique(pop_publisher) ]
    tmp_sample_ISBN = lib_df[ lib_df$도서명 %in% tmp_sample_books, 'ISBN']
    
    if (length(tmp_sample_books)>=20) {
      sample_idx = sample( length(tmp_sample_books), 20 )
      tmp_sample_books = tmp_sample_books[sample_idx]
      tmp_sample_ISBN = tmp_sample_ISBN[sample_idx]
    }
    
    compare_books = c( compare_books, tmp_sample_books )
    compare_ISBN = c( compare_ISBN, tmp_sample_ISBN )
  }
  
  ## 그래도 수집된 목록이 20권 미만인 경우, 전체 비인기도서들 중 무작위로 추가
  if (length(compare_books) < 20) {
    
    tmp_sample_df = lib_df[ !(lib_df$도서명 %in% compare_books),  ]
    n_sample = 20 - length(compare_books)
    sample_idx = sample( rownames(tmp_sample_df), n_sample )
    
    ### Debugging - 인덱싱 tmp_sample_df$도서명[sample_idx, ] 으로 잘못 하는 바람에 NA값이 무더기로 발생
    tmp_sample_books = tmp_sample_df[sample_idx, ]$도서명
    tmp_sample_ISBN = tmp_sample_df[sample_idx, ]$ISBN
  ...
  for (call_idx in 1:length(compare_books)) {
    ...
    if (call_idx%%50==0) {
      print('======= 50번째 도서 완료 =======')
      break_ans = readline('문제가 없습니까? 중단하려면 000 입력 >>> ')
      if (break_ans == "000") {
        break()
      }
    }
  ...
  recommend_list[[idx]] = list(popbook = tmp_popbook,
                               popISBN = tmp_popISBN,
                               recommend_books = tmp_recommend,
                               recommend_ISBN = tmp_recommend_ISBN,
                               recommend_score = tmp_mat[, "inner_products"])
...
## + 결과물 임시 저장
# saveRDS(res_list, '200103_19년12월_월곡꿈그림도서관_인기도서 10번 관련도서_키워드+가중치 리스트.rds')
# saveRDS(recommend_list, '200103_19년12월_월곡꿈그림도서관_인기도서별 추천도서 리스트.rds')
