...
# 2. 데이터 전처리 - 도서관 별로 특정 부가기호의 '미대출 도서 비율' 산정
...
for (tmp_file in src_file_vec) {
  ...
  tmp_df = tmp_df %>% filter(부가기호 == 7) %>% select(-부가기호)  # 아동도서
...
# 3. 데이터 전처리 - 도서관 별 '미대출 아동 도서 비율' df 생성
...
ggplot(data = plot_df, 
  ...
  geom_col(fill=c('red','red','gray','gray','gray')) +
  geom_text(aes(label=round(noLoan_percnt,1)), vjust=-0.3, size=5) +
  labs(title="2019년 12월 성북구 도서관별\n미대출 아동 도서 비율 (상위 5개 도서관)", y = '미대출 아동 도서 비율(%)', x="") +
  ylim(0, 40) + 
...
# ggsave("200103_19년 12월 성북구 도서관별 미대출 아동 도서 비율.png", width = 9, height = 6)
...
# ggsave("200103_19년 12월 성북구 도서관별 미대출 아동 도서 비율_barh.png", width = 9, height = 6)
