...
for (lib in names(sb_lib_list)) {
  ...
  temp_ratio = length( temp_df[ temp_df['대출건수'] < 1 ] ) / length(temp_df$도서명) * 100
...
big_ratio_vec = ratio_vec[ratio_vec > 40]
names = names(gu_lib_list)[ratio_vec > 40]
# names = substr( names, 1, 7 ); length(unique(names)) == length(names)
big_ratio_vec; names

barplot(big_ratio_vec ~ names,
        main = "도봉구 도서관별 버려진 도서 비율 (40% 이상)",
        xlab = "도서관", ylab = "버려진 도서 비율")
...
barplot(ratio_vec ~ names,
        main = "강북구 도서관별 버려진 도서 비율",
        xlab = "도서관", ylab = "버려진 도서 비율")
